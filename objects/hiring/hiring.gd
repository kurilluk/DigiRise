class_name Hiring extends Control

@onready var background = %HeadBackground
# different activator
@onready var slot_menu: SlotMenu = %HR
# new aplicator
@onready var newcomers: SlotMenu = %Newcomers
...
func reset_menu():
	_is_activated = false
	_is_done = false
	newcomers.clear_slots()
	newcomers.TYPE = MM.TYPES.EMPTY
...
func update_expanses():
	...
	#expanses += newcomers.get_prices_sum()
	if expanses > 0:
		_income = - expanses
...
### slot changed to newcomers - overriden
func get_employees() -> Array[int]:
	#slot_menu.upskill_slots(0) # TODO get slot/trainer level
